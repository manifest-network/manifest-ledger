...
| `ProviderWithdrawable` | Ordered best-effort execution estimate for the current ACTIVE-lease page. Failed lease simulations are discarded and successful virtual effects feed later leases; no query state commits. Shared tenant credit is counted once within the page, and pages are not additive. Only forward `limit <= 100` is comparable to one provider withdrawal. After commit, advance the query with its prior first-unread cursor and the transaction with its prior last-processed cursor; never interchange them. |
