...
func (server msgServer) Mint(goCtx context.Context, msg *types.MsgMint) (*types.MsgMintResponse, error) {
	...
	senderIsSudoAble := server.IsAdminFunc(ctx, msg.Sender)
	isSudo := sudoEnabled && senderIsSudoAble

	if isSudo {
		if err := server.ExtraSudoAllowedCheckFunc(ctx); err != nil {
			return nil, err
		}
	} else {
		...
		if !denomExists {
			return nil, types.ErrDenomDoesNotExist.Wrapf("denom: %s. sudoEnabled: %v senderIsSudoAble: %v", msg.Amount.Denom, sudoEnabled, senderIsSudoAble)
		}
