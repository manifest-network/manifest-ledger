...
func (suite *KeeperTestSuite) TestMintDenomMsg() {
	...
	someInflation := sdkmath.LegacyMustNewDecFromStr("0.15")
	...
	for _, tc := range []struct {
		...
		// Sudo Mints
		{
			desc:                  "successful sudo mint executed by the poa admin",
			amount:                10,
			mintDenom:             "unique",
			admin:                 suite.TestAccs[0].String(),
			poaAdmin:              suite.TestAccs[0].String(),
			inflation:             zeroInflation, // works at 0% inflation
			expectedMessageEvents: 1,
		},
		{
			desc:      "invalid sudo mint due to inflation >0%",
			amount:    10,
			mintDenom: "unique",
			admin:     suite.TestAccs[0].String(),
			poaAdmin:  suite.TestAccs[0].String(),
			inflation: someInflation,
		},
		{
			...
			mintDenom: "unique",
			admin:     suite.TestAccs[0].String(),
			poaAdmin:  suite.TestAccs[1].String(),
			inflation: zeroInflation,
		},
	} {
		suite.Run(fmt.Sprintf("Case %s", tc.desc), func() {
			...
			if tc.poaAdmin != "" {
				p := poa.DefaultParams()
				p.Admins = []string{tc.poaAdmin}
				suite.App.POAKeeper.SetParams(suite.Ctx, p)
			}
